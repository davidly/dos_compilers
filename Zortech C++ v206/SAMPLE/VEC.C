...
vec_t to,from;
{
  ...
  if (!(to && from && vec_numbits(to) == vec_numbits(from)))
	printf("to = x%x, from = x%x, numbits(to) = %d, numbits(from) = %d\n",
		to,from,vec_numbits(to),vec_numbits(from));
#endif
...
#ifdef DEBUG
#if LPTR
  printf(" Vec x%lx, numbits %d dim %d",v,vec_numbits(v),vec_dim(v));
#else
  printf(" Vec x%x, numbits %d dim %d",v,vec_numbits(v),vec_dim(v));
#endif
  if (v)
  {	fputc('\t',stdout);
