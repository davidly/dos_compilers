...
/* $Header: file.c,v 1.13 88/07/14 13:28:39 dietz Exp $ */
...
#define DIR 0x10		/* sub-directory attribute	*/
	...
	mem_free(p);
    if (f)
    {	files_pp = (char **) mem_malloc(sizeof(char *));
	if (!files_pp)
	    return 0;
#if DLC || __ZTC__
	*files_pp = mem_strdup(f->name);
#else
	*files_pp = mem_strdup(findbuf.name);
#endif
	assert(*files_pp);
	nfiles++;
#if DLC || __ZTC__
...
#endif
	{	files_pp = (char **) mem_realloc(files_pp,sizeof(char **) *
			(nfiles + 1));
		assert(files_pp);
#if DLC || __ZTC__
		files_pp[nfiles] = mem_strdup(f->name);
#else
		files_pp[nfiles] = mem_strdup(findbuf.name);
#endif
		assert(*files_pp[nfiles]);
		nfiles++;
	}
    }
    *Nfiles_p = nfiles;
